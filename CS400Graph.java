...
public class CS400Graph<T> implements AdvancedGraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	...
    	PriorityQueue<Path> targetPath = new PriorityQueue<Path>();
    	
    	if(vertices.containsKey(start) && vertices.containsKey(end)) {
    		paths.add(new Path(vertices.get(start)));
    	} else {
    		throw new NoSuchElementException("No vertex containinng start or end");
    	}
    	
    	while(paths.size() != 0) { //while more paths to explore
    		Path currPath = paths.poll();
    		Vertex currNode = currPath.end;
    		for(Edge e: currNode.edgesLeaving) {
    			if(!visited.containsKey(e.target.data)) {
    				Path newPath = new Path(currPath, e); //all paths to unvisited nodes
    				paths.add(newPath);
    				if(newPath.start.data == start && newPath.end.data == end) {
    					targetPath.add(newPath); //is a path to target node
    				}
    			}
    		}
    		visited.put(currNode.data, currNode);
    	}
